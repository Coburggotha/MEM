...
#include <stdlib.h>
#include <stdint.h>
#include <math.h>   // sqrt 사용 (소벨 코드에 필요)
...
int main() {
    ...
    const char* edgeFile = "output_edge.bmp";     // (추가) 엣지 BMP
    const char* memFile = "output_image.mem";    // (문구에만 사용)
    ...
    FILE* inFile = NULL;
    FILE* outFile = NULL;
    FILE* edgeOutFile = NULL;    // (추가) 엣지 BMP 파일 포인터
    FILE* memOutFile = NULL;     // (선언만 유지)
